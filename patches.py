...
def patch_filter(name: str, obj: Any):
    ...
    """
    cond = (
        not (isinstance(obj, types.ModuleType) or name.startswith("_"))
        or name == "__init__"
    )
...
class CustomGSS_greedyPlugin:
    def __init__(self: GSS_greedyPlugin, mem_size=200, mem_strength=5, input_size=[]):
        """
        ...
        self.ext_mem_list_d = torch.LongTensor(mem_size).fill_(0)  # added
    ...
    def before_training_exp(
        ...
        memory = list(zip(temp_x_tensors, temp_d_tensors, temp_y_tensors))  # added
    ...
    def after_forward(
        ...
        if place_left <= 0:  # buffer full
            ...
            self.ext_mem_list_d[curr_idx : curr_idx + offset].data.copy_(
                updated_mb_d
            )  # added
...
class CustomEWCPlugin:
    def compute_importances(
        self: EWCPlugin, model, criterion, optimizer, dataset, device, batch_size, num_workers=4
    ):
        """
